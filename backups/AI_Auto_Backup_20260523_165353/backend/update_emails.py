...
for i in range(0, len(lines) - 1, 2):
    ...
    def normalize(n):
        return n.lower().replace(' ', '').replace('ı','i').replace('ş','s').replace('ğ','g').replace('ü','u').replace('ö','o').replace('ç','c')
        
    email_map[normalize(name)] = email

print(f"Loaded {len(email_map)} emails attached mapping")
...
for doc in docs:
    ...
    norm_name = name.lower().replace(' ', '').replace('ı','i').replace('ş','s').replace('ğ','g').replace('ü','u').replace('ö','o').replace('ç','c')
    ...
    if match_email and data.get('email') != match_email:
        doc.reference.update({'email': match_email})
        print(f"Updated {name} -> {match_email}")
        updated_count += 1
