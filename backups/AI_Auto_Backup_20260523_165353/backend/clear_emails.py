...
for i in range(0, len(lines) - 1, 2):
    valid_emails.add(lines[i+1].lower())
...
for doc in docs:
    ...
    if current_email and current_email.lower() not in valid_emails:
        doc.reference.update({'email': ''})
        cleared_count += 1
        print(f"Cleared auto-generated email for {data.get('name')}")

print(f"Total cleared: {cleared_count}")
