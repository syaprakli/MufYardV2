...
function AuditListItem({ audit, onExportWord, onEdit, isSelected, onToggleSelect, task, onUpdate, onDelete }: { audit: AuditType, onExportWord: () => void, onEdit: () => void, isSelected: boolean, onToggleSelect: () => void, task?: Task, onUpdate: (id: string, updates: Partial<AuditType>) => void, onDelete: () => void }) {
    ...
    return (
        <Card className={`p-6 flex items-center justify-between transition-all group shadow-sm bg-white border-border/60 rounded-2xl relative ${isSelected ? 'border-red-500/50 ring-2 ring-red-500/20' : 'hover:border-primary/50 hover:shadow-xl'}`}>
            ...
            <div className="flex items-center gap-4 relative z-10">
                ...
                <div className="flex gap-2">
                    ...
                    <div className="relative">
                        ...
                        {isMenuOpen && (
                            <div 
                                className="absolute right-0 top-full mt-2 w-64 bg-white rounded-xl shadow-2xl border border-slate-200 z-[100] p-2 flex flex-col gap-1 ring-1 ring-black/5"
                                onClick={(e) => e.stopPropagation()}
                            >
                                <button 
                                    className="flex items-center gap-2 text-left px-3 py-2.5 text-sm font-bold hover:bg-slate-50 text-slate-800 rounded-lg transition-colors border-b border-slate-50 mb-1"
                                ...
                                {!task && (
                                    <button 
                                        className="flex items-center gap-2 text-left px-3 py-2 text-sm font-medium hover:bg-slate-50 text-slate-700 rounded-lg transition-colors"
