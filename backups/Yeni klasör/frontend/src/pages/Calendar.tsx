        ...
        <div className="space-y-8 animate-in fade-in duration-500">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    ...
                    <h1 className="text-3xl font-black text-slate-900 tracking-tight">Takvim</h1>
                    <p className="text-slate-500 text-sm font-medium mt-1">Denetim programınızı ve önemli tarihleri yönetin.</p>
                </div>
            </div>
                ...
                <div className="xl:col-span-3 card p-6">
                    ...
                    <div className="grid grid-cols-7 gap-px bg-border border border-border rounded-lg overflow-hidden shadow-sm">
                        ...
                        {Array.from({ length: firstDay }).map((_, i) => (
                            <div key={`empty-${i}`} className="bg-slate-50 min-h-[100px] border-t border-l border-border" />
                        ))}
                        {Array.from({ length: daysInMonth }).map((_, i) => {
                            ...
                            return (
                                <div
                                    ...
                                    className={cn(
                                        "bg-white min-h-[100px] p-2 hover:bg-muted/30 transition-colors cursor-pointer border-t border-l border-border relative group",
                        ...
                        {Array.from({ length: 42 - firstDay - daysInMonth }).map((_, i) => (
                            <div key={`empty-end-${i}`} className="bg-slate-50 min-h-[100px] border-t border-l border-border" />
                        ))}
                    </div>
                </div>
                    ...
                    <Card className="p-6">
                        ...
                        <div className="space-y-3">
                            {upcoming.length > 0 ? upcoming.map(t => (
                                <div key={t.id} className="flex items-start gap-3 p-2 bg-slate-50 rounded-lg hover:bg-slate-100 transition-all">
                                    <div className="w-1.5 min-h-[40px] bg-primary rounded-full flex-shrink-0 mt-1" />
                                    <div className="flex-1 min-w-0">
                                        <p className="text-xs font-bold text-slate-800 truncate" title={t.rapor_kodu}>{t.rapor_kodu}</p>
                                        <p className="text-[10px] text-slate-500 font-medium truncate">{t.rapor_adi}</p>
            ...
            >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 min-h-[350px]">
                    ...
                    <div className="flex flex-col h-full md:border-r border-border md:pr-8">
                        ...
                        <textarea
                            className="w-full flex-1 p-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-medium focus:ring-4 focus:ring-primary/10 outline-none resize-none mb-4"
                            ...
                            {selectedDay && tasks.filter(t => isDateInTaskRange(year, month, selectedDay, t)).map(t => {
                                ...
                                return (
                                    <div key={t.id} className={cn(
                                        ...
                                        isStart ? "bg-slate-50 border-slate-200 border-l-primary" : "bg-red-50/50 border-red-100 border-l-red-500"
                                    )}>
                                        ...
                                        <p className="text-slate-900 font-bold text-sm leading-tight mb-1 mt-1">{t.rapor_kodu}</p>
                                        <p className="text-slate-500 text-[10px] font-medium">{t.rapor_adi}</p>
                                    </div>
                                );
                            })}
                            ...
                            {notesLoading ? (
                                ...
                                selectedNotes.map(n => (
                                    <div key={n.id} className="p-4 bg-primary/5 rounded-xl border border-primary/10 relative group">
                                        <p className="text-slate-700 font-medium text-sm pr-6 leading-relaxed whitespace-pre-wrap break-words">{n.text}</p>
