...
def _build_model(api_key: str, model_name: str, system_instruction: str | None = None):
    """Verilen API key ve model adı ile geçici bir GenerativeModel oluşturur."""
    genai.configure(api_key=api_key)
    kwargs = {}
    if system_instruction:
        kwargs["system_instruction"] = system_instruction
    return genai.GenerativeModel(model_name or "gemini-2.0-flash", **kwargs)
...
async def _get_user_ai_settings(uid: str) -> tuple[str, str, float, str]:
    ...
    try:
        ...
        if doc.exists:
            ...
            # ai_model (Ayarlar sayfası) önce, fallback: gemini_model (API Key bölümü)
            model = (data.get("ai_model") or data.get("gemini_model") or "gemini-2.0-flash").strip()
            temperature = float(data.get("ai_temperature") or 0.7)
            user_system_prompt = (data.get("ai_system_prompt") or "").strip()
            return key, model, temperature, user_system_prompt
    except Exception:
        pass
    return "", "gemini-2.0-flash", 0.7, ""
...
{('\n\nKULLANICI ÖZEL TALİMATLARI:\n' + user_system_prompt) if user_system_prompt else ''}"""
...
"""
    ...
    async def _get_legislation_context(self, user: Dict[str, Any] | None = None) -> str:
        ...
        try:
            ...
            for doc in docs:
                ...
                # Dosya varsa oku (lokal veya Storage)
    ...
    async def _scan_local_mevzuat(self) -> List[str]:
        ...
        try:
            ...
            for rel_path, abs_path in files[:10]:  # Max 10 dosya
                text = await self._extract_file_text(abs_path)
                if text:
