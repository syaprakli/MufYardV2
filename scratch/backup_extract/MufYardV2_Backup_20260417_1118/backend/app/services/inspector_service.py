...
class InspectorService:
    ...
    async def sync_from_excel(file_path: str = None) -> Dict[str, Any]:
        ...
        Sadece: Başmüfettiş, Müfettiş, Müfettiş Yardımcısı ünvanlarını alır.
        """
        import os
        import pandas as pd
        import hashlib
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                email = str(row.get('E-MAİL', '')).strip() if not pd.isna(row.get('E-MAİL')) else f"{name.lower().replace(' ', '.')}@gsb.gov.tr"
