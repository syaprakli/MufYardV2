...
class AuditService:
    ...
    async def get_all_audits(user_id: Optional[str] = None, user_email: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if not user_id and not user_email:
            docs = await asyncio.to_thread(lambda: audits_ref.where('is_public', '==', True).stream())
            return [ {**doc.to_dict(), 'id': doc.id} for doc in docs]

        # 2. Admin/Demo bypass
        admin_id = "mufettis@gsb.gov.tr"
        if user_id == admin_id or user_email == admin_id or user_id == "admin":
            docs = await asyncio.to_thread(lambda: audits_ref.order_by('created_at', direction='DESCENDING').stream())
            return [ {**doc.to_dict(), 'id': doc.id} for doc in docs]
        ...
        queries = [
            ...
            audits_ref.where('is_public', '==', True)
        ]
    ...
    async def get_audit_versions(audit_id: str) -> List[Dict[str, Any]]:
        ...
        docs = await asyncio.to_thread(lambda: versions_ref.order_by('created_at', direction='DESCENDING').stream())
