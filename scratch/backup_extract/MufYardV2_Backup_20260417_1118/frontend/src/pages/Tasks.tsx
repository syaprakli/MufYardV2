...
const labelStyle: React.CSSProperties = { display: "block", fontSize: "0.65rem", fontWeight: 900, color: "#64748b", letterSpacing: "0.15em", marginBottom: "0.4rem", textTransform: "uppercase", fontFamily: "'Outfit', sans-serif" };
const inputStyle: React.CSSProperties = { width: "100%", padding: "0.8rem 1rem", border: "1px solid #e2e8f0", borderRadius: "1rem", fontSize: "0.875rem", outline: "none", fontFamily: "'Inter', sans-serif", boxSizing: "border-box", background: "white", transition: "all 0.2s" };
const tdStyle: React.CSSProperties = { padding: "1.2rem 1rem", fontSize: "13px", verticalAlign: "middle" };
const overlayStyle: React.CSSProperties = { position: "fixed", inset: 0, zIndex: 99999, display: "flex", alignItems: "center", justifyContent: "center", backgroundColor: "rgba(0,0,0,0.5)", backdropFilter: "blur(4px)" };
const modalBoxStyle: React.CSSProperties = { background: "white", borderRadius: "1rem", boxShadow: "0 25px 60px rgba(0,0,0,0.3)", width: "100%", maxWidth: "500px", padding: "1.75rem" };
...
export default function Tasks() {
    ...
    const handleOpenReportSelector = async (task: Task) => {
        try {
            setLoading(true);
            // Kapsayıcı bir arama için user_id olmadan (veya admin moduyla) çekilebilir 
            // ama şimdilik mevcut yetki dahilindeki raporları baz alıyoruz.
            const allAudits = await fetchAudits(effectiveUid);
            const associated = allAudits.filter(a => a.task_id === task.id);
    ...
    const handleCreateAuditFromTask = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Görevden ekip üyelerini miras al
            ...
            // Eğer yeni bir ek rapor ise doğrudan düzenleyiciye git
            if (newAudit.report_seq && newAudit.report_seq > 1) {
    ...
    const handleSablonSec = async (taskId: string, sablonName: string) => {
        try {
            ...
            const existingAudits = await fetchAudits(effectiveUid);
            const taskAudits = existingAudits.filter(a => a.task_id === taskId);
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div className="flex bg-slate-200/50 p-1.5 rounded-xl">
                    <button 
                        ...
                        className={`px-6 py-2.5 rounded-lg font-bold text-xs transition-all tracking-widest ${activeTab === 'kisisel' ? 'bg-white text-primary shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        Özel Kayıtlarım
                    </button>
                    <button 
                        onClick={() => setActiveTab("ortak")}
                        className={`px-6 py-2.5 rounded-lg font-bold text-xs transition-all tracking-widest ${activeTab === 'ortak' ? 'bg-white text-primary shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        Kurumsal Arşiv
                    </button>
                </div>
            ...
            {activeTab === 'kisisel' && (
                <div style={{ background: "white", borderRadius: "1rem", boxShadow: "0 4px 20px rgba(0,0,0,0.08)", padding: "1.5rem", marginBottom: "1.5rem", border: "1px solid #e2e8f0" }}>
                            ...
                            <div ref={inspectorDropdownRef} style={{ position: "relative" }}>
                                ...
                                <div
                                    ...
                                    style={{
                                        ...
                                        flexWrap: "wrap",
                                        gap: "0.3rem",
                                        borderColor: showInspectorDropdown ? "#0f172a" : "#e2e8f0",
                                        boxShadow: showInspectorDropdown ? "0 0 0 3px rgba(15,23,42,0.08)" : "none"
                                    }}
                                >
                                ...
                                {showInspectorDropdown && (
                                    <div style={{
                                        ...
                                        borderRadius: "0.75rem", boxShadow: "0 12px 40px rgba(0,0,0,0.12)",
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            ...
                                                            <div style={{ flex: 1, minWidth: 0 }}>
                                                                <div style={{ fontSize: "0.82rem", fontWeight: 700, color: "#0f172a", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{inspector.name}</div>
                                                                <div style={{ fontSize: "0.68rem", color: "#94a3b8", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{inspector.email}</div>
                                                            </div>
                                                        </div>
                                                    );
                                                })
                                        ...
                                        {/* Footer */}
                                        <div style={{ padding: "0.5rem 1rem", background: "#f8fafc", borderTop: "1px solid #f1f5f9", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                            <span style={{ fontSize: "0.72rem", color: "#64748b", fontWeight: 700 }}>{form.assigned_to.length} kişi seçildi</span>
                                            <button type="button" onClick={() => setShowInspectorDropdown(false)} style={{ fontSize: "0.72rem", fontWeight: 700, color: "#0f172a", background: "none", border: "none", cursor: "pointer" }}>Tamam</button>
                                        </div>
                                    </div>
                                )}
                            </div>
                        ...
                        <div style={{ marginTop: "1.25rem", borderTop: "1px solid #f1f5f9", paddingTop: "1.25rem" }}>
                            <button
                                ...
                                style={{
                                    ...
                                    letterSpacing: "0.03em", boxShadow: "0 4px 16px rgba(15,23,42,0.3)"
                                }}
                            >
                        ...
                        {invitations.map(inv => (
                            <div key={inv.id} className="bg-amber-50/50 border border-amber-100 rounded-2xl p-5 flex flex-col justify-between group hover:bg-amber-50 transition-all shadow-sm">
                                <div>
                                    ...
                                    <h4 className="font-bold text-slate-800 text-sm mb-1">{inv.rapor_adi}</h4>
            ...
            {/* Görevler Tablosu */}
            <div style={{ background: "white", borderRadius: "1rem", boxShadow: "0 4px 20px rgba(0,0,0,0.08)", border: "1px solid #e2e8f0", overflow: "hidden" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "1.5rem 1.75rem", borderBottom: "1px solid #f1f5f9" }}>
                    <h3 style={{ fontSize: "1.25rem", fontWeight: 900, fontFamily: "'Outfit', sans-serif", color: "#0f172a", letterSpacing: "-0.02em" }}>Görev Listesi</h3>
                        ...
                        <thead>
                            <tr style={{ background: "#f8fafc/50" }}>
                                {["Rapor No", "Görev Adı", "Tür", "Kalan", "Durum", "İşlemler"].map(col => (
                                    <th key={col} style={{ padding: "1.2rem 1rem", textAlign: "left", fontSize: "0.68rem", fontWeight: 900, color: "#64748b", letterSpacing: "0.15em", borderBottom: "1px solid #e2e8f0", textTransform: "uppercase", fontFamily: "'Outfit', sans-serif" }}>
                                        {col}
                                    </th>
                                ))}
                            </tr>
                                ...
                                filtered.map(task => {
                                    ...
                                    return (
                                        <React.Fragment key={task.id}>
                                            <tr style={{ borderBottom: isExpanded ? "none" : "1px solid #f1f5f9", background: isExpanded ? "#f8fafc" : "white" }} className="hover:bg-slate-50 transition-colors">
                                                <td style={tdStyle}><span style={{ fontWeight: 900, color: "#3b82f6", fontSize: "11px", letterSpacing: "0.02em", fontFamily: "'Outfit', sans-serif" }}>{task.rapor_kodu}</span></td>
                                                <td style={tdStyle}><span style={{ fontWeight: 700, color: "#1e293b", fontSize: "13px", fontFamily: "'Inter', sans-serif" }}>{task.rapor_adi}</span></td>
                                                <td style={tdStyle}><span style={{ fontWeight: 700, color: "#64748b", fontSize: "11px", fontFamily: "'Inter', sans-serif" }}>{task.rapor_turu}</span></td>
                                                <td style={tdStyle}>
                                                    {sureInfo ? (
                                                        <span style={{ 
                                                            fontSize: "0.75rem", 
                                                ...
                                                <td style={tdStyle}>
                                                    ...
                                                    >
                                                        {RAPOR_DURUMLARI.map(s => <option key={s} value={s} style={{ color: "#0f172a", background: "white" }}>{s}</option>)}
                                                    </select>
                                                </td>
                                                <td style={tdStyle}>
                                                        ...
                                                        <div style={{ backgroundColor: "white", padding: "1.25rem", borderRadius: "0.75rem", border: "1px solid #e2e8f0" }}>
                                                            ...
                                                            <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                                                                ...
                                                                <div style={{ display: "flex", gap: "0.5rem", marginTop: "0.5rem" }}>
                                                                    ...
                                                                    />
                                                                    <button onClick={() => handleAddStep(task)} style={{ padding: "0.4rem 1rem", backgroundColor: "#0f172a", color: "white", borderRadius: "0.5rem", fontSize: "0.8rem", fontWeight: 700 }}>Ekle</button>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </td>
                ...
                <div style={overlayStyle} onClick={(e) => { if (e.target === e.currentTarget) setShowReportSelector(null); }}>
                    <div style={{...modalBoxStyle, maxWidth: "600px"}} onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-between mb-6">
                            <div>
                                <h3 className="text-xl font-black font-outfit text-slate-800">BAĞLI RAPORLAR</h3>
                                <p className="text-xs font-bold text-slate-400 mt-1 uppercase tracking-widest">{showReportSelector.rapor_kodu}</p>
                            </div>
                            <button onClick={() => setShowReportSelector(null)} className="p-2 hover:bg-slate-100 rounded-xl transition-colors">
                        ...
                        <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar mb-6">
                            {taskAudits.sort((a,b) => (a.report_seq || 0) - (b.report_seq || 0)).map(audit => (
                                <div 
                                    ...
                                    className="p-4 rounded-2xl border border-slate-100 bg-slate-50/50 hover:bg-slate-50 hover:border-slate-200 cursor-pointer transition-all group"
                                >
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-10 rounded-xl bg-white shadow-sm flex items-center justify-center text-primary/40 group-hover:bg-primary group-hover:text-white transition-all">
                                                <FileText size={20} />
                                            </div>
                                            <div>
                                                ...
                                                <h4 className="font-bold text-slate-700 leading-none">{audit.title}</h4>
                                            </div>
                                        </div>
                    ...
                    <div style={modalBoxStyle} onClick={e => e.stopPropagation()}>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                            {Object.keys(RAPOR_SABLONLARI).map(s => (
                                <button key={s} onClick={() => handleSablonSec(showSablonModal, s)} style={{ padding: "1rem", borderRadius: "0.75rem", border: "1px solid #e2e8f0", backgroundColor: "white", cursor: "pointer", textAlign: "left", fontWeight: 600 }}>
                                    {s}
                                </button>
                            ))}
                        </div>
                ...
                <div style={overlayStyle} onClick={(e) => { if (e.target === e.currentTarget) setEditingTask(null); }}>
                    <div style={{...modalBoxStyle, maxWidth: "600px"}} onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-between mb-6 border-b border-slate-50 pb-4">
                            <div>
                                <h3 className="text-xl font-black font-outfit text-slate-800">GÖREVİ DÜZENLE</h3>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex gap-3 pt-4 border-t border-slate-50">
                                <button onClick={() => setEditingTask(null)} className="flex-1 h-12 rounded-xl border border-slate-100 font-bold text-slate-500 hover:bg-slate-50 transition-all text-xs uppercase tracking-widest">
                                    İPTAL
                                </button>
                                <button 
                ...
                <div style={overlayStyle} onClick={(e) => { if (e.target === e.currentTarget) setDeleteConfirmId(null); }}>
                    <div style={modalBoxStyle} onClick={e => e.stopPropagation()}>
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: '1.5rem' }}>
                            ...
                            <div>
                                <h3 style={{ fontSize: '1.25rem', fontWeight: 800, color: '#0f172a', fontFamily: 'Outfit, sans-serif' }}>Görevi Sil?</h3>
                                <p style={{ fontSize: '0.875rem', color: '#64748b', marginTop: '0.5rem', fontWeight: 500 }}>Bu görevi silmek istediğinizden emin misiniz? Bu işlem geri alınamaz.</p>
                            </div>
                            <div style={{ display: 'flex', gap: '1rem', width: '100%' }}>
                                <button 
                                    onClick={() => setDeleteConfirmId(null)}
                                    style={{ flex: 1, padding: '0.85rem', borderRadius: '0.75rem', border: '1px solid #e2e8f0', backgroundColor: 'transparent', fontWeight: 700, cursor: 'pointer', fontFamily: 'Inter, sans-serif' }}
                                >
                                    İptal
                                </button>
                                <button 
            ...
            {isNewAuditModalOpen && (
                <div style={overlayStyle} onClick={(e) => { if (e.target === e.currentTarget) setIsNewAuditModalOpen(null); }}>
                    <div style={{ ...modalBoxStyle, maxWidth: '600px' }} onClick={e => e.stopPropagation()}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', borderBottom: '1px solid #f1f5f9', paddingBottom: '1rem' }}>
                            <div>
                                <h3 style={{ fontSize: '1.25rem', fontWeight: 800, color: '#0f172a', fontFamily: 'Outfit, sans-serif' }}>Yeni Rapor Başlat</h3>
                                <p style={{ fontSize: '0.75rem', color: '#64748b', fontWeight: 600, marginTop: '0.25rem' }}>Denetim detaylarını onaylayın veya güncelleyin.</p>
                            </div>
                            <button onClick={() => setIsNewAuditModalOpen(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8' }}><X size={20} /></button>
                        </div>
                        <form onSubmit={handleCreateAuditFromTask} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                            <div>
                                <label style={labelStyle}>İşle İlişkili Görev</label>
                                <div style={{ ...inputStyle, backgroundColor: '#f8fafc', color: '#64748b', cursor: 'not-allowed', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                                ...
                                <div>
                                    <label style={labelStyle}>Denetim Yeri</label>
                                    <input 
                                        type="text" 
                                        style={inputStyle}
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                                <div>
                                    <label style={labelStyle}>Denetim Tarihi</label>
                                    <input 
                                        type="text" 
                                        style={inputStyle}
                                        value={newAudit.date}
                                ...
                                <div>
                                    <label style={labelStyle}>Taslak Şablon</label>
                                    <select 
                                        style={inputStyle}
                                        value={newAudit.template}
                                        onChange={(e) => setNewAudit({...newAudit, template: e.target.value})}
                                    >
                                        {Object.keys(RAPOR_SABLONLARI).map(s => <option key={s} value={s}>{s}</option>)}
                                    </select>
                                </div>
                            </div>
                            <div style={{ display: 'flex', gap: '1rem', paddingTop: '0.5rem' }}>
                                <button 
                                    ...
                                    style={{ flex: 1, height: '3.5rem', borderRadius: '1rem', border: '1px solid #e2e8f0', backgroundColor: 'transparent', fontWeight: 700, cursor: 'pointer', fontFamily: 'Inter, sans-serif' }}
                                >
                                    İptal
                                </button>
                                <button 
                                    type="submit"
                                    disabled={saving}
                                    style={{ flex: 1, height: '3.5rem', borderRadius: '1rem', border: 'none', backgroundColor: '#0f172a', color: 'white', fontWeight: 700, cursor: 'pointer', fontFamily: 'Inter, sans-serif', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}
                                >
