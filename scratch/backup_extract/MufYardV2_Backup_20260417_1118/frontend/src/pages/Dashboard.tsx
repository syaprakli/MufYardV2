...
const COLORS = {
    blue: { bg: '#eff6ff', border: '#dbeafe', text: '#1e40af', chart: '#60a5fa' },
    green: { bg: '#f0fdf4', border: '#dcfce7', text: '#166534', chart: '#4ade80' },
    rose: { bg: '#fff1f2', border: '#ffe4e6', text: '#9f1239', chart: '#fb7185' },
    amber: { bg: '#fffbeb', border: '#fef3c7', text: '#92400e', chart: '#fbbf24' },
    indigo: { bg: '#eef2ff', border: '#e0e7ff', text: '#3730a3', chart: '#818cf8' },
    slate: { bg: '#f8fafc', border: '#f1f5f9', text: '#475569', chart: '#94a3b8' }
};
...
4. Öneriler ve aksiyon planı`;
    ...
    return (
        <div className="max-w-[1600px] mx-auto space-y-8 animate-in fade-in duration-500 pb-12 pr-4 pl-2">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    ...
                    <h1 className="text-3xl font-black text-slate-900 tracking-tight">
                        Hoş Geldiniz, <span className="text-primary">{user?.displayName?.split(' ')[0] || "Müfettiş"}</span>
                    </h1>
                ...
                <div className="flex items-center gap-2">
                    <div className="relative" ref={filterRef}>
                        <Button 
                            ...
                            size="sm" 
                            className={cn(
                                "bg-white border-slate-100 border font-bold text-[11px] rounded-xl px-4 h-11 transition-all",
                        ...
                        {/* ─── Filtre Dropdown ─── */}
                        {showFilters && (
                            <div className="absolute right-0 top-14 z-50 bg-white border border-slate-100 rounded-2xl shadow-xl p-5 w-[320px] space-y-4 animate-in fade-in slide-in-from-top-2 duration-200">
                                ...
                                <div>
                                    ...
                                    <select 
                                        ...
                                        className="w-full rounded-xl border border-slate-100 bg-slate-50/50 text-sm font-semibold text-slate-700 px-3 py-2.5 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all"
                                    >
                                ...
                                <div>
                                    ...
                                    >
                                        <option value="Tümü">Tümü</option>
                                        {RAPOR_DURUMLARI.map(d => <option key={d} value={d}>{d}</option>)}
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                {/* Sistem Duyuruları */}
                <Card className="p-6 border border-slate-200 shadow-sm bg-white xl:col-span-2">
                    <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center gap-2">
                            <Bell size={16} className="text-primary" />
                            <h3 className="font-black text-[11px] text-slate-800 uppercase tracking-widest">Sistem Duyuruları</h3>
                        </div>
                        <Button 
                            variant="link" 
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        {data?.news?.map((news: any, index: number) => (
                            <div 
                                ...
                                className="bg-white p-4 rounded-xl border border-slate-100 hover:border-primary/20 transition-all cursor-pointer group"
                            >
                                ...
                                <p className="text-[11px] font-bold text-slate-700 leading-relaxed group-hover:text-primary transition-colors">{news.title}</p>
                            </div>
                        ))}
                    </div>
                </Card>
                ...
                <Card className="p-5 border border-slate-200 shadow-sm bg-white">
                    <div className="flex items-center gap-2 mb-4">
                        ...
                        <h3 className="font-black text-[10px] text-slate-800 uppercase tracking-widest">Hızlı Erişim</h3>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                        {[
                            ...
                            <a
                                ...
                                className="flex items-center gap-2.5 bg-white px-3 py-2.5 rounded-lg border border-slate-100 hover:border-primary/30 transition-all group"
                            >
                                <link.icon size={14} style={{ color: link.color }} className="flex-shrink-0" />
                                <span className="text-[11px] font-bold text-slate-600 group-hover:text-primary transition-colors truncate">{link.name}</span>
                            </a>
                        ))}
                    </div>
                </Card>
            ...
            <Card className="p-0 overflow-hidden shadow-none border-slate-100 bg-white">
                <div className="p-6 border-b border-slate-50 flex justify-between items-center bg-white">
                    <h3 className="font-black text-xl text-slate-900 font-outfit tracking-tight">Görevler (Özet)</h3>
                    <Button 
                        variant="outline" 
                        size="sm" 
                        className="bg-white border-slate-100 border text-slate-400 font-black text-[10px] uppercase tracking-widest rounded-xl px-4 h-10 hover:border-primary/20 hover:text-primary transition-all"
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                        <thead className="bg-[#f8fafc]/50 text-slate-400/80 font-black uppercase text-[11px] tracking-[0.15em] border-b border-slate-100 font-outfit">
                            <tr>
                ...
                <div className="p-4 bg-white flex justify-end items-center border-t border-slate-50">
                    <Button 
                        variant="link" 
                        size="sm" 
            ...
            {showAnalysis && createPortal(
                <div className="fixed inset-0 z-[1000] flex justify-end">
                    ...
                    />
                    {/* Drawer */}
                    <div className="relative w-full max-w-xl bg-white/90 backdrop-blur-xl shadow-2xl animate-in slide-in-from-right duration-300 flex flex-col">
                        {/* Header */}
                        <div className="flex items-center justify-between p-5 border-b border-slate-100 bg-white/70 backdrop-blur-xl">
                            <div className="flex items-center gap-3">
                                <div className="p-2 bg-primary/10 rounded-xl">
                                    <Bot size={18} className="text-primary" />
                                </div>
                                <div>
                                    <h3 className="font-black text-sm text-slate-900 tracking-tight">AI Performans Analizi</h3>
...
function TableRow({ code, title, type, status, diff, rawColor, rawStatusColor }: any) {
    return (
        <tr className="hover:bg-slate-50/50 transition-colors border-b border-slate-50 last:border-0">
            <td className="px-6 py-5 font-black text-primary text-[10px] tracking-wider font-outfit">{code}</td>
            <td className="px-6 py-5 font-bold text-slate-800 text-[13px] font-inter">{title}</td>
            <td className="px-6 py-5 text-[11px] font-bold text-slate-500/80 font-inter">{type}</td>
...
function StatCard({ title, value, trend, icon: Icon, isAlert }: any) {
    return (
        <Card 
            className={cn(
                "p-5 border-none shadow-none transition-all group",
                isAlert ? "bg-rose-50 border border-rose-100" : "bg-white border border-slate-100"
            )}
        >
            <div className="flex items-center justify-between">
                <div className="space-y-1">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{title}</p>
                    <p className={cn("text-2xl font-black tracking-tighter", isAlert ? "text-rose-600" : "text-slate-800")}>{value}</p>
                    <p className={cn("text-[9px] font-bold flex items-center gap-1", isAlert ? "text-rose-400" : "text-emerald-500")}>
                        {isAlert ? <AlertCircle size={10} /> : <ArrowUpRight size={10} />} {trend}
                    </p>
                </div>
                <div className={cn("p-3 rounded-xl transition-all", isAlert ? "bg-rose-100" : "bg-slate-50 group-hover:bg-primary/5")}>
                    <Icon size={20} className={cn(isAlert ? "text-rose-500" : "text-slate-400 group-hover:text-primary")} />
                </div>
            </div>
        </Card>
    );
