...
import { X, Minus, MessageSquare, Send, Paperclip, Smile, Image as ImageIcon, Search, FileText } from 'lucide-react';
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3 bg-[#f8fafc]">
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-center p-6 text-slate-400 space-y-2">
            ...
            <p className="text-[9px] font-medium leading-relaxed opacity-60">Mesajlar bu oturum özelindedir ve kaydedilmez.</p>
          </div>
        )}
        {messages.map((msg) => {
          ...
          return (
            <div key={msg.id} className={`flex flex-col ${isOwn ? 'items-end' : 'items-start'}`}>
              ...
              <div className={`max-w-[85%] ${msg.content ? 'p-3' : 'p-1'} rounded-2xl text-xs font-medium shadow-sm leading-relaxed ${isOwn ? 'bg-primary text-white rounded-tr-none' : 'bg-white text-slate-700 border border-slate-100 rounded-tl-none'}`}>
