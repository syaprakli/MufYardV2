...
interface ShareModalProps {
    ...
    sharedRoles?: Record<string, "view" | "comment" | "edit">;
    onShareWithRoles?: (newSharedWith: string[], newSharedRoles: Record<string, "view" | "comment" | "edit">) => void;
    enableRoles?: boolean;
    title?: string;
}
...
export default function ShareModal({
    ...
    onShareWithRoles,
    enableRoles = false,
    title = "Paylaşımı Yönet"
}: ShareModalProps) {
    const { user, profile } = useAuth();
    const [search, setSearch] = useState("");
    const [selected, setSelected] = useState<string[]>(sharedWith);
    const [roles, setRoles] = useState<Record<string, "view" | "comment" | "edit">>(sharedRoles || {});
    ...
    const toggleUser = (id: string) => {
        setSelected(prev => {
            ...
            const next = exists ? prev.filter(u => u !== id) : [...prev, id];
            if (!exists && enableRoles) {
                setRoles((r) => ({ ...r, [id]: r[id] || "view" }));
            }
            if (exists && enableRoles) {
                setRoles((r) => {
                    const copy = { ...r };
    ...
    const setUserRole = (id: string, role: "view" | "comment" | "edit") => {
    ...
    const handleSave = () => {
        ...
        const selectedRoleMap: Record<string, "view" | "comment" | "edit"> = {};
        users
            .filter((u) => selected.includes(u.id) || (u.email && selected.includes(u.email)))
            .forEach((u) => {
                const role = roles[u.id] || "view";
                (u.identityKeys || [u.id, u.email].filter(Boolean)).forEach((k: string) => {
                    if (!currentUserKeys.includes(String(k).toLowerCase())) {
                        selectedRoleMap[k] = role;
                    }
                });
            });

        if (enableRoles && onShareWithRoles) {
            onShareWithRoles(expandedKeys, selectedRoleMap);
        } else {
            ...
            <div style={modalBoxStyle} onClick={e => e.stopPropagation()}>
                ...
                <div style={listContainerStyle}>
                    {loading ? (
                        ...
                        const selectedRole = roles[user.id] || "view";
                        return (
                            <div 
                                key={user.id} 
                                style={{
                            ...
                            >
                                ...
                                <div style={{ display: "flex", alignItems: "center", gap: "0.4rem" }}>
                                    {enableRoles && isSelected && (
                                        <select
                                            value={selectedRole}
                                            onChange={(e) => setUserRole(user.id, e.target.value as "view" | "comment" | "edit")}
                                        ...
                                        >
                                            ...
                                            <option value="comment">Yorumla</option>
