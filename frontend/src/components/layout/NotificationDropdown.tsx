...
const getIcon = (type: string) => {
    ...
    return (
        <div className="fixed sm:absolute top-16 sm:top-full right-4 sm:right-0 mt-2 sm:mt-4 w-[calc(100vw-32px)] sm:w-[400px] bg-white/95 backdrop-blur-xl border border-slate-200 rounded-3xl shadow-2xl shadow-slate-200/50 overflow-hidden z-[60] animate-in fade-in slide-in-from-top-2 duration-200">
            {/* Header */}
                ...
                {filteredNotifications.length > 0 ? (
                    filteredNotifications.map((notif) => (
                        <div 
                            ...
                            onClick={() => {
                                markAsRead(notif.id);
                                if ((notif.type as string) === 'collaboration' && (notif as any).chat_room_id) {
                                    const title = notif.title.replace('Yeni Mesaj: ', '');
                                    openChat((notif as any).chat_room_id, title, 'dm');
                                } else if ((notif.type as string) === 'task' || notif.type === 'task_invite') {
                                    navigate('/tasks');
                                }
                            }}
