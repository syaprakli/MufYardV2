...
export default function AdminLicenses() {
    ...
    const toggleSelectAll = () => {
        const pageKeys = paginatedLicenses.map(l => l.key);
    ...
    useEffect(() => {
        setCurrentPage(1);
    }, [searchTerm, filter, durationFilter, licenses]);
                        ...
                        <thead>
                            <tr className="bg-white">
                                <th className="px-8 py-6 text-left w-12">
                                    <input 
                                        type="checkbox"
                                        checked={paginatedLicenses.length > 0 && paginatedLicenses.every(l => selectedKeys.includes(l.key))}
