...
import {
    ...
    Printer, Copy, Camera, Download, Building2
} from "lucide-react";
...
const EVRAK_TALEP_MADDELERI = [
    "Kuruluş ve tescil işlemlerine ilişkin evrak",
    "Çek Karnesi",
    "Sulh ve İbra Sözleşmeleri",
    "Varsa ayni yardım teslim belgesi (dağıtım tutanakları, ayni bağış alındı belgesi)",
    "Başta tasdikli Yönetim Kurulu Karar defteri olmak üzere tüm yasal defterler ve varsa tutulan yardımcı defterler",
    "Bankalar nezdindeki tüm TL ve YP vadeli, vadesiz mevduat, yatırım hesapları, kredi kartı pos cihazı kullanımının bağlı olduğu hesaplar, kullanılan kredi hesapları (denetim dönemi içerisinde işlem görmüş ancak kapatılmış banka hesapları dahil), Kasa hesabı varsa denetime başlanıldığı gün itibariyle buna ilişkin mevcut durum tutanakları",
    "Tüzük, Esas Sözleşme ve Genel Kurul toplantı tutanakları (olağan ve olağanüstü kurul toplantıları)",
    "İç Denetim Raporları (Denetim dönemi itibari ile)",
    "Varsa Bağımsız Denetim Raporları",
    "Finansal Tablolar (Denetim dönemi itibariyle konsolidasyona tabi olan kulüp ve şirketlerde konsolide finansal tablolar ile konsolidasyon tarihinde konsolide olanlara ait olan tablolar birlikte)",
    "Yasal defterlere ve yardımcı defterlere intikal etmiş işlemlere ait tüm evrak",
    "Bağlı ortaklık, iştirak ve iktisadi işletmeler varsa bu şirketlerin kuruluş ve varsa değişiklik ticaret sicil gazeteleri",
    "Bağlı bulunulan İl Müdürlüklerine verilen beyannameler",
    "Yönetim Kurulu üyelerinin ve personelin dosyaları",
    "Hasılat ve gelir kalemleri (UEFA, sporcu satışı, lisanslı ürün, bilet, yayın, loca, sponsor vb. gerçekleşen ile kayıtların uygunluğu, finansal tablolara uygunluk, taraflar vb.)",
    "Futbolcu, Teknik Direktör ve Temsilci Sözleşmeleri (Tek taraflı-karşılıklı fesihler, tahkim kararları, geciken maaşlar, tazminatlar, vekalet ücretleri ve faiz ödemeleri, bonservis bedeli, amortisman giderleri vb.)",
    "Bilet satış işlemlerine ilişkin sözleşme ve gelirlerin takibi, bedelsiz verilen biletler",
    "Sponsorluklara ilişkin kayıt ve belgeler",
    "Tahsisli, kiralık ve mülk olan menkul (araçlar vb.) ve gayrimenkullerin listesi",
    "Spor okullarına ilişkin iş ve işlemler",
    "Vadesinde Ödenmeyen Vergi ve SGK Borçlarına dair kayıtlar",
    "Lisansiyerlerden Tahsil Edilen Royalty Bedellerine ilişkin kayıtlar",
    "Verilen avanslar, alacakların takip ve tahsili, dava karşılıklarına ilişkin kayıtlar",
    "İnşaatlar, yapım işlerine ilişkin kayıtlar (Haklar, sözleşmeler, tedarikçilere ödemeler, yüklenici kontrolü vb.)",
    "Kulüp içi talimatların mevcudiyeti, güncel mevzuata uygunluğuna dair bilgiler",
    "UEFA ve TFF tarafından Kulübe kesilen cezalar vb. kayıtları",
    "Muhatap kalınan icra takipleri, faiz ödemeleri, aleyhteki davalar vb. hakkındaki kayıtlar",
    "Satın alma ve gider işlemleri ile ticari ilişkilerde tespit edilen aksaklıklar ve istisnai durumlara ilişkin kayıtlar (yetki durumu, kalıntı değerler, fiziki mevcudiyet, envanter kaydı vb.)",
    "Nakit ve nakit benzerleri, ticari alacaklar ve borçlar, tahsil kabiliyeti, beklenen zarar, işlemlerin tutar ve niteliği vb. hakkındaki kayıtlar",
    "Stoklara ilişkin kayıtlar (devir hızı, kayıt sistemi, uygulanan programlama işlerliği vb., fatura/irsaliye)",
    "Sabit kıymetlere ilişkin kayıtlar (yeterli açıklama, sınıflandırma, mülkiyet, yapılan tahminler, kredi vb.)"
];
...
export default function DenetimSpor() {
    ...
    const activeTab = "spor" as string;
    ...
    const [localAuditData, setLocalAuditData] = useState<any>({
        ...
        evrakTalep: {
            ...
            denetimYili: new Date().getFullYear().toString(),
            teslimSuresi: "7",
    ...
    // Bu sayfa YALNIZCA Spor Kulüpleri Denetimi sayfasi oldugu icin soru seti daima spor'dur!
    const taskTabId = "spor";

    // Questions answered by the user for preview modal
    const previewQuestions = useMemo(() => {
        const questions = AUDIT_TEMPLATES.spor || [];
    ...
    const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            savePhotoToLocalDevice(file, `Spor_${selectedReport?.title || "Denetim"}`);
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        {photos.length > 0 && (
                            <button
                                type="button"
                                onClick={() => exportPhotosToComputer(photos, `${selectedReport?.title || "Spor_Denetimi"}_Fotograflar`)}
                            ...
                            return (
                                ...
                                >
                                    <div className="relative aspect-video sm:aspect-square overflow-hidden bg-slate-100 dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800">
                                        ...
                                        <button
                                            type="button"
                                            onClick={() => exportPhotosToComputer([url], `${selectedReport?.title || "Spor_Denetimi"}_Foto_${index + 1}`)}
        ...
        return (
            <div className="flex-1 grid grid-cols-1 xl:grid-cols-12 gap-6 xl:overflow-hidden h-auto xl:h-full">
                ...
                {activeQuestionForTenkit && (
                    <div className="xl:col-span-5 flex flex-col xl:max-h-full xl:overflow-hidden h-auto border border-slate-200 dark:border-slate-800 rounded-2xl bg-slate-50 dark:bg-slate-955 p-4 relative">
                    ...
                    {tenkitResults.length === 0 ? (
                        ...
                        tenkitResults.map(item => (
                            ...
                            >
                                ...
                                <div className="text-[10px] text-slate-600 dark:text-slate-400 leading-relaxed line-clamp-3 bg-slate-50 dark:bg-slate-950/5 p-2 rounded font-medium border border-slate-100 dark:border-slate-800/20" dangerouslySetInnerHTML={{ __html: item.standard_remark }} />
                            </div>
                        ))
                    )}
                </div>
    ...
    const ensureReportInitialized = async () => {
        if (selectedReport) return selectedReport;
        
        // Initialize report automatically
        const defaultReportNo = selectedTask?.rapor_kodu || "DENETİM-1";
        const newAuditPayload = {
            task_id: selectedTask!.id,
            title: defaultReportNo,
            location: "",
            date: new Date().toLocaleDateString("tr-TR"),
            inspector: profile?.full_name || user?.displayName || user?.email?.split('@')[0] || "Müfettiş",
            status: "Devam Ediyor",
            report_content: `<h1>${defaultReportNo}</h1><p>Denetim bulguları ve tespitleri buraya kaydedilecektir.</p>`,
            owner_id: user?.uid,
            assigned_to: [user?.uid].filter(Boolean) as string[],
            report_seq: 1,
            report_created: false
        ...
        const modules = [
            {
                ...
                description: "Denetimin başında kulüpten istenecek yasal defterler, tüzükler ve mali evrakın resmi talep yazısı modülü.",
                icon: FileText,
                color: {
                ...
                stats: "31 Madde / Şablon"
            },
            {
                id: "checklist",
                title: "Denetim Kontrol Listesi",
                description: "Spor kulübünün idari ve mali süreçlerini incelemek ve bulgu girmek için 105 maddelik denetim formu.",
                icon: ClipboardCheck,
                color: {
            ...
            },
            {
                ...
                description: "Denetim yerinden, belgelerden veya kulüp defterlerinden çekilerek sisteme eklenen görseller.",
                icon: Camera,
                color: {
                    ...
                    {modules.map(mod => {
                        ...
                        return (
                            <div
                                key={mod.id}
                                onClick={async () => {
                                    if (mod.id !== "info" && !selectedReport) {
                                        try {
                                            await ensureReportInitialized();
                                        } catch (err) {
                                            return;
                                        }
                                    }
                                    setActiveDetailTab(mod.id as any);
                                }}
                            ...
                            >
                                <div className="space-y-3">
                                    ...
                                    <div className="space-y-1">
                                        <h4 className="text-sm font-black text-slate-900 dark:text-white tracking-tight group-hover:text-primary transition-colors">
                                            {mod.title}
                                        </h4>
    ...
    const renderEvrakTalebiTab = () => {
        ...
        const denetimYiliStr = evrakTalep.denetimYili || new Date().getFullYear().toString();
        const teslimSuresiStr = evrakTalep.teslimSuresi || "7";
        const kulupAdiStr = selectedTask?.rapor_adi ? selectedTask.rapor_adi.toUpperCase() : "....... SPOR KULÜBÜ";
        ...
        const getPlainText = () => {
            ...
            text += `Konu: Teftiş/Denetim İşleri\n\n`;
            text += `${kulupAdiStr} SPOR KULÜBÜ BAŞKANLIĞINA\n\n`;
            text += `İlgi: a) ${olurTarihiStr} tarihli ve ${olurSayisiStr} sayılı Makam Oluru.\n`;
            text += `      b) ${gorevTarihiStr} tarihli ve ${gorevSayisiStr} sayılı Görevlendirme Yazısı.\n\n`;
            text += `İlgi (a)'da yer alan Bakanlık Makamı Oluru ve ilgi (b)'de kayıtlı Rehberlik ve Denetim Başkanlığının Görev Emri uyarınca, ${denetimYiliStr} yılı Spor Kulüpleri ve Spor Anonim Şirketleri Genel Denetimi kapsamında aşağıda belirtilen tüm bilgi ve belgelerin madde bazında tasniflenmiş onaylı suretlerinin denetime hazır hale getirilerek ${teslimSuresiStr} (yedi) gün içinde Müfettişliğe teslim edilmesini rica ederim.\n\n`;
        ...
        const handlePrint = () => {
            ...
            printWindow.document.write(`
                ...
                <body>
                    ...
                    <div class="subject">
                        <strong>Konu:</strong> Teftiş/Denetim İşleri
                    </div>
                    
                    <div class="recipient">
                        ${kulupAdiStr} SPOR KULÜBÜ BAŞKANLIĞINA
                    </div>
                    
                    <div class="reference">
                        <strong>İlgi:</strong><br />
                        a) ${olurTarihiStr} tarihli ve ${olurSayisiStr} sayılı Makam Oluru.<br />
                        b) ${gorevTarihiStr} tarihli ve ${gorevSayisiStr} sayılı Görevlendirme Yazısı.
                    </div>
                    
                    <div class="content">
                        İlgi (a)'da yer alan Bakanlık Makamı Oluru ve ilgi (b)'de kayıtlı Rehberlik ve Denetim Başkanlığının Görev Emri uyarınca, ${denetimYiliStr} yılı Spor Kulüpleri ve Spor Anonim Şirketleri Genel Denetimi kapsamında aşağıda belirtilen tüm bilgi ve belgelerin madde bazında tasniflenmiş onaylı suretlerinin denetime hazır hale getirilerek ${teslimSuresiStr} (yedi) gün içinde Müfettişliğe teslim edilmesini rica ederim.
                    </div>
        ...
        const handleDownloadWord = () => {
            const kulupAdiClean = (selectedTask?.rapor_adi || "Evrak_Talep_Yazisi").replace(/[\s/\\?%*:|"<>]/g, "_");
            const filename = `Spor_Kulubu_Evrak_Talep_Yazisi_${kulupAdiClean}.doc`;
            ...
            <div className="flex flex-col gap-6">
                ...
                <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start">
                    ...
                    <div className="xl:col-span-5 space-y-6">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="space-y-1.5 col-span-2">
                                    ...
                                    <input
                                        ...
                                        placeholder="Örn: Bakanlık Müfettişi, Bakanlık Müfettiş Yardımcısı..."
                                        className="w-full px-3 py-2 text-xs rounded-lg border border-slate-250 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-900 dark:text-white outline-none focus:ring-1 focus:ring-blue-500 font-medium"
                                    />
                                </div>
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <input
                                        ...
                                        placeholder="Örn: E-41444738-660..."
                                        className="w-full px-3 py-2 text-xs rounded-lg border border-slate-250 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-900 dark:text-white outline-none focus:ring-1 focus:ring-blue-500 font-medium"
                                    />
                                </div>
                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-black uppercase tracking-wider text-slate-400 dark:text-slate-500">Denetim Yılı</label>
                                    <input
                                        type="text"
                        ...
                        {/* Checklist selection */}
                        <div className="bg-slate-50 dark:bg-slate-950 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-4 flex flex-col max-h-[400px]">
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`flex items-start gap-2.5 p-2 rounded-xl border text-left cursor-pointer transition-all ${
                                                isChecked
                                                    ? "bg-blue-600/5 dark:bg-blue-950/10 border-blue-200 dark:border-blue-900/40"
                    ...
                    <div className="xl:col-span-7 space-y-3">
                        <div className="flex justify-between items-center px-1">
                            <span className="text-[10px] font-black uppercase tracking-wider text-slate-400 dark:text-slate-500">Resmi Belge Önizleme</span>
                            <span className="text-[9px] font-bold bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded text-slate-500">{evrakTalep.selectedItems?.length || 0} Evrak Seçili</span>
                        </div>
                        
                        <div className="bg-white text-slate-900 border border-slate-200 dark:border-slate-850 p-8 shadow-md rounded-2xl font-serif text-[12px] leading-relaxed max-w-full select-text min-h-[500px] overflow-x-auto relative">
                            {/* Watermark/Official letter styling simulation */}
                            <div className="text-center font-bold mb-6 text-[13px] border-b border-double border-slate-200 pb-4">
                                T.C.<br />
                                GENÇLİK VE SPOR BAKANLIĞI<br />
                                Bakanlık Müfettişliği
                            </div>
                            ...
                            <div className="text-center font-bold my-5 text-[13px] tracking-wide">
                                {kulupAdiStr} SPOR KULÜBÜ BAŞKANLIĞINA
                            </div>
                            ...
                            <p className="text-justify mb-5" style={{ textIndent: "20px" }}>
                                İlgi (a)'da yer alan Bakanlık Makamı Oluru ve ilgi (b)'de kayıtlı Rehberlik ve Denetim Başkanlığının Görev Emri uyarınca, <strong>{denetimYiliStr}</strong> yılı Spor Kulüpleri ve Spor Anonim Şirketleri Genel Denetimi kapsamında aşağıda belirtilen tüm bilgi ve belgelerin madde bazında tasniflenmiş onaylı suretlerinin denetime hazır hale getirilerek <strong>{teslimSuresiStr} (yedi)</strong> gün içinde Müfettişliğe teslim edilmesini rica ederim.
                            </p>
                            
                            <div className="font-bold mb-2">TALEP EDİLEN BİLGİ VE BELGELER:</div>
                            ...
                            <div className="mt-8 float-right text-center font-bold pr-6">
                                {inspectorName}<br />
                                <span className="text-[10px] font-normal text-slate-500 dark:text-slate-400">Bakanlık Müfettişi</span>
                            </div>
            ...
            <div className={`flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 bg-white dark:bg-slate-900/30 backdrop-blur-md border border-slate-100 dark:border-slate-900/50 rounded-2xl p-3 sm:p-4 flex-shrink-0 justify-between shadow-sm ${activeDetailTab !== "hub" ? "hidden" : "flex"}`}>
                <div className="flex items-center gap-3 min-w-0">
                    <button
                        ...
                        className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-slate-100 dark:bg-slate-955 border border-slate-200 dark:border-slate-800 text-slate-500 hover:text-slate-700 dark:hover:text-slate-350 hover:bg-slate-200 dark:hover:bg-slate-900 flex items-center justify-center transition-all duration-200 shrink-0"
                    ...
                    <div className="min-w-0">
                        <div className="flex items-center gap-1.5 text-[9px] font-black uppercase tracking-[0.2em] text-slate-400">
                            ...
                            <ChevronRight size={8} />
                            <span className="text-blue-500 font-bold truncate">Spor Kulüpleri Denetimi</span>
                        </div>
                        <h1 className="text-base sm:text-lg font-black text-slate-900 dark:text-white tracking-tight mt-0.5 truncate">Spor Kulüpleri Denetimi</h1>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto">
                    {selectedTaskId && (
                        <button
                            ...
                            title="Tüm Kulüpler / Görevler Listesini Aç"
                        >
                            <Building2 size={12} className="shrink-0" />
                            <span className="truncate">Kulüp Değiştir ({filteredTasks.length})</span>
                        </button>
                    )}
                    <button
                                                ...
                                                <div className="flex flex-col items-center justify-center py-12 text-center gap-3">
                                                    ...
                                                    <div className="flex flex-col gap-1 items-center">
                                                        ...
                                                        <p className="text-xs text-slate-400 mt-1 max-w-sm leading-relaxed">
                                                            Seçili denetim türünde (Spor Kulüpleri Denetimi) henüz bir görev oluşturulmamıştır. Denetime başlamak için önce "Görevler" sayfasından bu türde bir görev oluşturmalısınız.
                                                        </p>
                                                    </div>
                                                    <Button
                                                        onClick={() => {
