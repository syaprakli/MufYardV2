...
import { Users, Search, ChevronLeft, ChevronRight, UserCheck, UserX, Loader2, MoreVertical, ShieldAlert, Shield } from "lucide-react";
...
export default function AdminInspectors() {
    ...
    const loadData = async () => {
        setLoading(true);
        try {
            const [insp, profs] = await Promise.all([
                fetchInspectors(),
                fetchAllProfiles(),
            ]);
            setInspectors(insp);
    ...
    // Sadece uid veya email ile eşleştir — isim eşleştirmesi hatalı pozitife yol açar
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="flex items-center gap-2 flex-wrap">
                ...
                <button
                    onClick={() => { setFilterMode("registered"); setCurrentPage(1); }}
                    className={cn(
                        "flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-black transition-all border",
                ...
                <button
                    onClick={() => { setFilterMode("unregistered"); setCurrentPage(1); }}
                    className={cn(
                        "flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-black transition-all border",
                            ...
                            return (
                                ...
                                >
                                    <div className="flex items-center gap-4 flex-1 min-w-0">
                                        {/* Avatar / Initial */}
                                        <div className={cn(
                                            "w-11 h-11 rounded-xl flex items-center justify-center font-black text-sm shrink-0",
                                            registered
                                        ...
                                        <div className="min-w-0">
                                            ...
                                            <div className="flex items-center gap-2 flex-wrap">
                                                <p className={cn(
                                                    ...
                                                    registered ? "text-emerald-700 dark:text-emerald-400" : "text-slate-700 dark:text-slate-300"
                                                )}>
                                                    {ins.name}
                                                </p>
                                                {registered && (
                                                    <span className="text-[9px] bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 px-2 py-0.5 rounded-full font-black uppercase tracking-widest whitespace-nowrap">
                                                        ✓ Kayıtlı
                                                    </span>
                                                )}
                                            ...
                                            <div className="flex flex-wrap items-center gap-2 mt-0.5">
                                                ...
                                                {ins.title && (
                                                    <span className="text-[9px] bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded font-bold text-slate-500">
                                                        {ins.title}
                                                    </span>
                                                )}
                                                {ins.extension && (
                                                    <span className="text-[9px] bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded font-bold text-slate-500">
                                                        Dahili: {ins.extension}
                                                    </span>
                                                )}
                                                {ins.phone && (
                                                    <span className="text-[9px] bg-emerald-50 dark:bg-emerald-950/20 px-2 py-0.5 rounded font-bold text-emerald-600">
                                                        {ins.phone}
                                                    </span>
                                                )}
                                            </div>
                                    ...
                                    {/* Kayıtlı kullanıcılar için rol menüsü */}
                                    {registered && profile && (
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {[...Array(Math.min(totalPages, 7))].map((_, i) => (
                            <button
                                ...
                                className={cn("w-9 h-9 rounded-xl text-xs font-bold transition-all", currentPage === i + 1 ? "bg-primary text-white shadow-lg shadow-primary/20" : "text-slate-400 hover:bg-muted")}
                            >
                                {i + 1}
                            </button>
                        ))}
